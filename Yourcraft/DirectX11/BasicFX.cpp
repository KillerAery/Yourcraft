...
public:
	...
	ComPtr<ID3D11ShaderResourceView> textrues;
};
...
bool BasicFX::InitAll(ComPtr<ID3D11Device> device)
{
	...
	// 创建顶点布局
	HR(device->CreateInputLayout(VertexPositionNormalTexture::InputElements, ARRAYSIZE(VertexPositionNormalTexture::InputElements),
...
{
	pImpl->cbObjDrawing.data.world = W;
	pImpl->cbObjDrawing.data.worldInvTranspose = XMMatrixTranspose(XMMatrixInverse(nullptr, W));
