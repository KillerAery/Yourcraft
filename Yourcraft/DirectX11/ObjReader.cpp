	...
	for (;;)
	{
		...
		else if (wstr == L"f")
		{
			...
			for (int i = 0; i < 3; ++i)
			{
				...
				vertex.textureCoordinate = texCoords[vti[i] - 1];
...
bool ObjReader::ReadMbo(const wchar_t * mboFileName)
{
	...
	for (UINT i = 0; i < parts; ++i)
	{
		...
		fin.read(reinterpret_cast<char*>(objParts[i].vertices.data()), vertexCount * sizeof(DirectX::VertexPositionNormalTexture));
...
bool ObjReader::WriteMbo(const wchar_t * mboFileName)
{
	...
	for (UINT i = 0; i < parts; ++i)
	{
		...
		if (vertexCount > 65535)
		{
			...
			// [顶点]32*顶点数 字节
			fout.write(reinterpret_cast<const char*>(objParts[i].vertices.data()), vertexCount * sizeof(DirectX::VertexPositionNormalTexture));
			// [索引]4*索引数 字节
...
void ObjReader::AddVertex(const DirectX::VertexPositionNormalTexture& vertex, DWORD vpi, DWORD vti, DWORD vni)
{
